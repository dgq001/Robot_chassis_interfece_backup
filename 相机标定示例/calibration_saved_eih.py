...
class Calibrator():  
    ...
    def calib(self, verbose: bool = True, save: bool = False, e2h: bool = False, cam_name:str="head") -> np.ndarray:  
        """  
        ...
        while i<36:  
            ...
            fname=f'/home/taizun/WorkSpace/calibration/calibrate_data/20240519_righthand/{i}/{cam_name}.jpg'#{cam_name}
            ...
            if ret:  
                ...
                corners_filename=f"/home/taizun/WorkSpace/calibration/calibrate_data/20240519_righthand_chessboard/corners_image_{i}_{cam_name}.png"
                ...
                #     [902.93416593, 0, 636.38878409],
                #     [0,902.65823373, 304.80344321],
                #     [0,0,1]])
                #读取出的内参
                color_intrinsics_mat=np.array([
                    [905.305, 0, 639.558],
                ...
                # # 获取存储的机械臂位姿
                # 眼在手上标定不需要求逆
                tcpfname=f'/home/taizun/WorkSpace/calibration/calibrate_data/20240519_righthand/{i-1}/arm_info.json'
                ...
                arm_rotmat=RT_array[:3,:3]

                arm_transes.append(arm_transe)
                arm_rotmats.append(arm_rotmat)
        ...
        R, t = cv2.calibrateHandEye(arm_rotmats, arm_transes, rvecs, tvecs, method=cv2.CALIB_HAND_EYE_PARK)  
        T = np.eye(4)  
        T[:3, :3] = R  
        T[:3, 3] = t[:, 0]  
        ...
        if save:  
            save_dir = os.path.join(os.getcwd(), f"{cam_name}_calib_results")  
            if not os.path.exists(save_dir):  
                os.makedirs(save_dir)  
            np.save(os.path.join(save_dir, f"calib_{cam_name}_RT.npy"), T)    
        return T  
...
if __name__ == "__main__":  
    ...
    T=calibrator.calib(save=True, cam_name="right_hand")
    print(T)
